...
- **Audit admissibility:** Category and overall geometric means use **finite** per-key admissibility only (non-finite keys excluded); categories with no finite keys are omitted from `per_category`. **`admissibility_level`** returns **Unknown** for non-finite scores. **RMS** (`_rms_scalar`) and weak-form chain RMS use **NaN-tolerant** reductions (`nanmean` / `nansum`); π-constant scale uses `nanmean` on absolute compare values.
...
- **Chain closure ``chain_output``:** ``AuditSpec`` / audit dicts support ``chain_output: state_derivative`` (default: LHS from ``d_<output>/dx`` in state) or ``fd_on_composition`` (LHS = finite difference of ``F(*args)`` on the mesh using ``x``/``t`` coords). **Studio:** selecting **Fourier conduction** with **`T`** in the NPZ injects an implied **scaling** audit for **`fo`** with ``fd_on_composition`` and ``predicted_spatial`` including **`alpha`** so **``scaling/fo/chain_dx``** can run without **`d_fo_dx`**. Tests: ``tests/test_closure_chain.py``, ``tests/test_studio_auto_config.py``.
- **Moju Studio — model-derived chain:** [`studio_model_derived_registry`](apps/moju_studio/studio_model_derived_registry.py) + `enrich_fragment_from_model_audits()` appends `derived_state_chain` steps when a constitutive audit matches a hand-maintained bridge (first entry: **`thermal_diffusivity`** → `alpha = k/(rho*cp)` for any **`groups`** spec that needs `alpha`, e.g. **`fo`**). Applied on Run and Dependency preview after config merge. Tests: `tests/test_studio_model_derived_registry.py`.
- **Derived state DSL:** `exp` and `pow` nodes in `moju.monitor.derived_state_chain` for nonlinear material laws (e.g. Arrhenius-style factors). **Moju Studio README** documents **Fourier conduction** (`fo` / `alpha` / `derived_state_chain`); Audit **Config** captions point to it.
- **Derived state chain (Phase B):** `MonitorConfig.derived_state_chain` — ordered `{"output_key", "expr"}` steps evaluated **before** `groups` / Path B FD / laws using a JSON-safe expression DSL (`moju.monitor.derived_state_chain`). `ResidualEngine.required_state_keys` and Studio dependency planning union `ref` dependencies and exclude chain-produced keys. Studio auto fragment + JSON override support `derived_state_chain`. Tests: `tests/test_derived_state_chain.py`.
...
- **Moju Studio — dependency planner:** `apps/moju_studio/studio_dependency_planner.py` derives required state keys, audit derivative keys, and law-FD prerequisites from the merged config fragment; **Config** tab **Dependency preview** expander; **Run** preflight + downloadable checklist append planner markdown (`preflight_checklist_with_dependency_plan`, `dependency_plan_for_path_b_run`). Built-in **NPZ key aliases** (e.g. `temperature` → `T`) with warning text. Tests: `tests/test_studio_dependency_planner.py`.
...
- **`moju.monitor.visualize_labels`** — `pretty_residual_key`, `pretty_category_name` for publication-style residual and category labels (e.g. `chain_dx` → “Spatial Consistency”).
...
- **Moju Studio — scaling audits:** Inferred **`predicted_temporal`** lists now exclude mesh **`t`** (and all **`x,y,z,t`** via **`_COORD_ALL`**), preventing spurious **`d_fo_dt`** / **`chain_dt`** on the time coordinate when **`t`** is in NPZ. README: transient **`T`** / **`t`** shape note for law FD fill.
...
- **Path B multi-axis chain + FD fill:** `AuditSpec.chain_spatial_axes` (default `["x"]`) drives `chain_dx` / `chain_dy` / `chain_dz` residuals and `required_derivative_keys()`. Optional `compute_residuals(..., auto_path_b_derivatives=True|PathBGridConfig)` fills missing `d_*_dx`/`_dy`/`_dz`/`_dt` via `jnp.gradient` on structured grids. Helpers `PathBGridConfig`, `fill_path_b_derivatives` in `moju.monitor`; `moju.monitor.derivative_keys` for key naming. Tests: `tests/test_path_b_derivatives.py`.
- **Path B law FD (optional):** `fill_path_b_derivatives(..., fill_law_recipes=True, laws_spec=...)` and `compute_residuals(..., auto_path_b_derivatives=..., fill_law_fd=True)` fill **registered** missing `Laws.*` inputs (e.g. `phi_laplacian`, `u_grad`, `T_t`) from primitives on the same structured grid (`moju.monitor.law_fd_recipes`, `list_law_fd_supported_laws()`). Rectilinear 2D/3D meshgrid coordinates are detected for JAX-compatible spacing. Tests: `tests/test_law_fd_recipes.py`. Examples: `examples/cookbook_path_b_fd_audit_pe.py`, `examples/cookbook_path_b_fd_law_laplace.py`.
