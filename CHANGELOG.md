...
- **Plotly `visualize` (single-figure):** **Enterprise header:** **Overall admissibility (final)** is merged into **`layout.title`** under the main title with **`<br>`** and **~22px** top margin on the subtitle block; main title **20px**, subtitle row **15px** (shared for training and test). Resolved **`figure_title`** strings get **first-word capitalization** via **`_visualize_capitalize_first_word`** in **`auditor`**. KPI category labels (**Governing / Constitutive / Scaling Score**) use a larger Indicator **`title`** font (**13px**). No separate paper annotation for the subtitle. When **`figure_title`** is empty, only the overall line is used as the title. Tighter **top margin** / **`title.pad`**; slimmer row-1 / row-2 fractions; **`vertical_spacing` ~0.057**, **`horizontal_spacing` ~0.065**, figure **height ~1760**, and **`update_xaxes(automargin=True, title.standoff=14)`** so x tick labels and axis titles clear the next row’s subplot titles. **Three KPI cards** use a **centered** row-2 grid with indicators at **cols 2, 4, 6**; **`align: center`** on indicators. Stray subplot titles for **Governing / Constitutive / Scaling / Duality Score** are stripped. **Spatial row heatmaps** share **`zmin`/`zmax`** in display space (log or linear R_norm) so governing and constitutive panels use the same color scale. Extra top margin and overall-admissibility **y-axis headroom** so the trend line is not clipped; category breakdown **reference line at 95%**; four-way header status tags (**HIGH** / **MODERATE** / **LOW** / **NON-ADM**); **Primary Issue** annotation only when any displayed category score is not above 95%. `format_admissibility_status_label` (e.g. Moju Studio captions) now returns the same phrases as `admissibility_level`.
...
- **Plotly `visualize` (single-figure, test mode):** **Four-row** grid: row 3 is **Category Breakdown** (left) and **Normalized residuals (test)** — one **`go.Bar`** for all **`bar_keys`** in user order with **per-key colors** (**`_residual_color_from_key`**: laws, constitutive, scaling/groups, data, other). Separate Governing/Constitutive residual rows are removed; **spatial** panels use **row 4** (heatmap **`meta.subplot_row`** updated accordingly). Training single-figure layout is unchanged (**five** rows, spatial on row 5).
...
- **Plotly `visualize` trend + heatmaps:** Training **Overall Admissibility** uses a **black** trend line, **`plotly_white`** / white plot background, and an explicit **full-domain white `add_shape`** under the line (no pink high-variance background tint—visual parity with residual panels). Heatmap **colorbars** use shared constants and **`align_heatmap_colorbars_to_subplot_domains`**: flush-right subplots get an **x-domain inset** so the bar clears the heatmap (with the same gap logic as left-column maps), including **forensic** and **Studio** cards (`Heatmap` `meta` carries subplot row/col).
