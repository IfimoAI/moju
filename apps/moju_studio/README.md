...
- **Dashboard** — card-based Plotly charts (law / category bars; law vs constitutive spatial **R_norm** heatmaps); optional combined multi-panel figure; optional `r_ref` / weights / `max_legend_keys` / π slider for the **next** run. Sidebar **heatmap colorscale** and **spatial axis** (`x` / `y` / `z`) apply to spatial heatmaps; session log append (sidebar) for multi-step runs; redraw expander with a subset of keys. The Python API still supports `visualize(..., mode="training"|"test")`.
- **Export** — JSON reports, optional **PDF ZIP** if `moju[report]` is installed.

## Model-derived `derived_state_chain` (auto)

Some **`Groups.*` inputs** (e.g. **`alpha`** for **`Groups.fo`**) can be filled automatically when you add the matching **constitutive audit** (`Models.*`): Studio runs [`enrich_fragment_from_model_audits`](apps/moju_studio/studio_model_derived_registry.py) after merging constants and **appends** JSON-safe `derived_state_chain` steps before `groups` / FD / laws.
...
The audit’s **`state_map`** supplies the state keys for `k`, `rho`, `cp` (identity map by default). If you already define the `output_key` in **`derived_state_chain`** or NPZ, nothing is duplicated. Other **`fo`** inputs (**`t`**, **`L`**) are unchanged — see **Time `t`** below.
...
**Easiest:** add **`thermal_diffusivity`** under Models — Studio auto-appends **`alpha`** from **`k`**, **`rho`**, **`cp`** when `fo` needs `alpha`, and the audit still checks consistency. Alternatively put **`alpha`** in the NPZ, or use a manual **`derived_state_chain`**.
...
**Chain residual (`scaling/fo/chain_dx`):** the monitor compares **∂(group output)/∂x** (LHS) to **Σᵢ (∂F/∂φᵢ)(∂φᵢ/∂x)** (RHS). Default LHS uses **`d_fo_dx`** from Path B FD on the **`fo`** field. When **`T`** is in the NPZ, Studio injects an implied **`fo`** scaling audit with **`chain_output: fd_on_composition`**, so the LHS is a **finite difference of `fo` on the mesh** (no **`d_fo_dx`** key required) while RHS still uses **`d_alpha_dx`** (and JAX **∂fo/∂α**). See **`AuditSpec.chain_output`** in `moju.monitor.config` and `compute_chain` in `moju.monitor.closure_registry`.

**Constitutive `d_alpha_dx` chain (`constitutive/thermal_diffusivity/.../chain_dx`):** the law-linked Fourier row compares **α implied from the heat equation** to **`Models.thermal_diffusivity(k, ρ, cₚ)`** via **`implied_delta`** and does **not** request spatial chain unless **`predicted_spatial`** is non-empty. Studio therefore adds **`predicted_spatial: ["alpha"]`** on that row when **`alpha`** is in the upload and you **did not** select the **`thermal_diffusivity`** model (avoids a duplicate constitutive row). The dependency planner then requires **`d_alpha_dx`** (or Path B auto-FD). If you **do** select **`thermal_diffusivity`** under Models, Studio skips this patch and chain keys follow the usual rule (**`k`**, **`rho`**, **`cp`** in the NPZ → spatial/temporal chain on those inputs).

**Temperature-dependent properties:** use **`derived_state_chain`** in Expert **MonitorConfig** JSON or in the **Optional JSON override** (auto mode). Steps run **before** `groups`, so `alpha` is available when `Groups.fo` runs. Put coefficients in **Constants JSON** and reference them with `{"op": "ref", "key": "cp"}`.
...
The DSL also supports **`exp`** and **`pow`** (see `moju.monitor.derived_state_chain`) for nonlinear laws such as Arrhenius-style factors.
