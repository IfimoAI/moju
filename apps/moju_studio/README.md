...
- **Audit / visualize** — optional `r_ref` and weights JSON; session log append for multi-step Plotly dashboards; sidebar **heatmap colorscale** and **spatial axis** (`x` / `y` / `z`) for R_norm heatmaps; redraw with a subset of keys. The Python API still supports `visualize(..., mode="training"|"test")`; Studio uses a single monitor layout without those labels.
