...
This produces `dist/moju-<version>.tar.gz` and `dist/moju-<version>-py3-none-any.whl` (e.g. `moju-0.4.3`).
...
1. Create an annotated tag for the version: `git tag -a v0.4.3 -m "Release 0.4.3"` (on the commit that has the version bump).
2. Push the tag: `git push origin v0.4.3`.
3. Create a GitHub Release from the tag (Releases → Draft a new release → choose tag `v0.4.3`, set as latest, publish). This triggers the Publish to PyPI workflow if the version is not already on PyPI.
