...
class TestLawFdMmsFd4:
    """MMS-style checks for default fd_order=4 on uniform structured grids."""

    def test_quartic_laplacian_matches_analytic_including_boundaries(self):
        ...
        assert "phi_laplacian" in out, w
        assert jnp.allclose(out["phi_laplacian"], expect, rtol=1e-4, atol=1e-4)
        assert jnp.allclose(out["phi_laplacian"][0], expect[0], rtol=1e-4, atol=1e-4)
        assert jnp.allclose(out["phi_laplacian"][-1], expect[-1], rtol=1e-4, atol=1e-4)

    def test_quartic_laplacian_2d_rectilinear_boundaries(self):
        nx, ny = 18, 16
        xs = jnp.linspace(0.0, 1.0, nx)
        ys = jnp.linspace(0.0, 1.0, ny)
        X, Y = jnp.meshgrid(xs, ys, indexing="ij")
        T = X**4 + Y**4
        expect = 12.0 * X**2 + 12.0 * Y**2
        state = {"T": T, "x": xs, "y": ys}
        laws = [
            {"name": "fourier_conduction", "state_map": {"T_laplacian": "T_laplacian"}}
        ]
        ...
        assert "T_laplacian" in out, w
        assert jnp.allclose(out["T_laplacian"], expect, rtol=2e-3, atol=2e-3)

    def test_sine_laplacian_refinement_rate_near_fourth_order(self):
        """Interior max error vs h ~ h^4; use float64 (float32 + small h amplifies roundoff)."""
        prev_x64 = jax.config.jax_enable_x64
        jax.config.update("jax_enable_x64", True)
        try:

            def interior_max_err(n: int) -> float:
                x = jnp.linspace(0.0, 1.0, n, dtype=jnp.float64)
                phi = jnp.sin(jnp.pi * x)
                analytic = -(jnp.pi**2) * phi
                state = {"phi": phi, "x": x}
                laws = [
                    {
                        "name": "laplace_equation",
                ...
                lap = out["phi_laplacian"]
                return float(jnp.max(jnp.abs(lap[2:-2] - analytic[2:-2])))

            e_coarse = interior_max_err(33)
            e_fine = interior_max_err(129)
            ratio = e_coarse / (e_fine + 1e-30)
            # h_fine / h_coarse = 32/128 = 1/4 => (1/4)^4 error drop => ratio ~ 256
            assert ratio > 100.0
            assert ratio < 400.0
        finally:
            jax.config.update("jax_enable_x64", prev_x64)

    def test_small_grid_falls_back_without_crashing(self):
        """n < 5 uses jnp.gradient-style 2nd-order path inside stencil helpers."""
        ...
        assert "phi_laplacian" in out, w
        assert jnp.all(jnp.isfinite(out["phi_laplacian"]))

    def test_fd_order_two_matches_double_gradient_reference(self):
        ...
        out, _ = fill_law_fd_from_primitives(
            ...
            laws,
            grid=PathBGridConfig(
                layout="meshgrid", spatial_dimension=1, steady=True, fd_order=2
            ),
        )
        assert jnp.allclose(out["phi_laplacian"], expect, rtol=1e-5, atol=1e-5)
