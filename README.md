...
moju is JAX-native, JIT-compiled, and fully differentiable—built for physics-informed neural networks (PINNs) and digital twins. Residuals and operators integrate with JAX autodiff so you can train networks to satisfy the equations. We build on the principle that **physics is the ground truth** and provide the "glass box" transparency needed to deploy AI in high-stakes settings (thermal management, flow simulation, and beyond).
